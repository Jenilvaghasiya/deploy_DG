...
export default function NewFeaturesTab() {
  const [form, setForm] = useState<FeatureFormState>(defaultState);
  const [loading, setLoading] = useState(false);
  const [isConfirmOpen, setIsConfirmOpen] = useState(false);
  const [errors, setErrors] = useState<Partial<FeatureFormState>>({});
  ...
  const validateDates = () => {
    const newErrors: Partial<FeatureFormState> = {};
    const now = new Date();
  ...
  const setField = <K extends keyof FeatureFormState>(k: K, v: FeatureFormState[K]) => {
    setForm((s) => {
      const newForm = { ...s, [k]: v };
  ...
  return (
    <div style={styles.Maincontainer}>
      <div style={styles.container}>
        <h2 style={styles.heading}>Announce New Feature</h2>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          <label style={styles.label}>
            ...
            <input
              ...
              required
              placeholder="e.g., New Dashboard Analytics"
            />
          </label>
          ...
          >
            {loading ? "Processing…" : "Schedule Announcement"}
          </button>
        </form>
      </div>
      ...
      {/* Confirmation modal */}
      <ConfirmationModal
        message={`Are you sure you want to schedule this announcement?\n⚠️ This will be emailed to ALL users at the specified time.`}
