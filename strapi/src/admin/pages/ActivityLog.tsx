...
const ActivityLog = () => {
  ...
  const [activityLogs, setActivityLogs] = useState<ActivityLog[]>([]); // Changed from tenants
  const [loadingLogs, setLoadingLogs] = useState(false); // Changed from loadingUsers
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.STRAPI_ADMIN_NODE_BACKEND_URL}/api/v1/strapi-admin/activity-logs?${params.toString()}`,
        {
          headers: {
...
const handleCansel = async () => {
  ...
  // Add these mapping objects for module and type names
  ...
  const TYPE_NAMES: any = {
    ...
    spam_post_deleted: "Spam Post Deleted",
  };

  // Function to generate description similar to the user activity component
  const generateDescription = (log: ActivityLog) => {
    const userName = "Admin";

    switch (log.type) {
      case "credit_added":
        return `${userName} added credits to a user ${log.metadata?.name || ""}'s account`;
      case "post_approved":
        return `${userName} approved a post ${log.metadata?.name || ""}`;
      case "post_rejected":
        return `${userName} rejected a post ${log.metadata?.name || ""}`;
      case "post_deleted":
        return `${userName} deleted a post ${log.metadata?.name || ""}`;
      case "review_approved":
        return `${userName} approved a review for post ${log.metadata?.name || ""}`;
      case "review_rejected":
        return `${userName} rejected a review for post ${log.metadata?.name || ""}`;
      case "review_deleted":
        return `${userName} deleted a review for post ${log.metadata?.name || ""}`;
      case "spam_post_deleted":
        return `${userName} deleted a spam post ${log.metadata?.name || ""}`;
      default:
        return log.message || "Admin action performed";
    }
  };
    ...
    <div style={strapiStyles.container}>
      ...
      <div style={strapiStyles.contentBox}>
        ...
        <div style={strapiStyles.header}>
          <h2 style={strapiStyles.title}>Acitvity Logs</h2>
        </div>
        ...
        <div style={strapiStyles.tableContainer}>
          {loadingLogs ? (
            <p style={strapiStyles.loadingText}>Loading logs...</p>
          ) : (
            <table style={strapiStyles.table}>
              <thead style={strapiStyles.tableHeader}>
                <tr>
                  <th style={strapiStyles.th}>Module</th>
                  <th style={strapiStyles.th}>Action</th>
                  <th style={strapiStyles.th}>Description</th>
                  <th
                    style={{
                      ...strapiStyles.th,
                      // cursor: "pointer",
              ...
              <tbody>
                {activityLogs.map((log) => (
                  <tr key={log._id} style={strapiStyles.tr}>
                    <td style={strapiStyles.td}>
                      {MODULE_NAMES[log.module] || log.module || "N/A"}
                    </td>
                    <td style={strapiStyles.td}>
                      {TYPE_NAMES[log.type] || log.type || "N/A"}
                    </td>
                    <td style={strapiStyles.td}>{generateDescription(log)}</td>
                ...
                {activityLogs.length === 0 && (
                  <tr>
                    <td colSpan={6} style={strapiStyles.emptyState} className="min-h-80">
