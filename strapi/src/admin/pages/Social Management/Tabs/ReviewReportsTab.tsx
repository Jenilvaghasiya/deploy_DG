...
import PostReportsModal from "../Modals/PostReportModal";
import axios from "axios";
import { MdOutlineOpenInNew } from "react-icons/md";
import ReviewReportsModal from "../Modals/ReviewReportModal";
...
const ReviewReportsTab = () => {
  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);
  const [limit] = useState(10);
  const [reviews, setReviews] = useState<SocialPost[]>([]);
  ...
  const fetchReviews = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.STRAPI_ADMIN_NODE_BACKEND_URL}/api/v1/strapi-admin/reviews/approved?${params.toString()}`,
        {
          headers: {
...
const handleDeleteReport = async (reviewId: string, reportId: string) => {
  try {
    await axios.delete(`${process.env.STRAPI_ADMIN_NODE_BACKEND_URL}/api/v1/strapi-admin/social-posts/review/report/delete/${reviewId}/${reportId}`, {
      headers: {
        "x-node-auth-token": process.env.STRAPI_ADMIN_NODE_AUTH_TOKEN,
      },
    });
    ...
    <div style={strapiStyles.container}>
      ...
      <div style={strapiStyles.contentBox}>
        ...
        <div style={strapiStyles.header}>
          <h2 style={strapiStyles.title}>Post Review Reports</h2>
        </div>
          ...
          {loadingReviews ? (
            ...
            <table style={strapiStyles.table}>
              <thead style={strapiStyles.tableHeader}>
                <tr>
                    <th style={strapiStyles.th}>User</th>
                    <th style={strapiStyles.th}>Title</th>
                    <th style={strapiStyles.th}>Ratings</th>
                    <th style={strapiStyles.th}>Report Count</th>
                    <th
              ...
              <tbody>
              {reviews.map((review) => (
                <tr key={review._id} style={strapiStyles.tr}>
                  <td style={strapiStyles.td}>{review?.user_id?.nick_name || "Unknown"}</td>
                  <td style={strapiStyles.td}>{review.post_id.title}</td>
                  <td style={strapiStyles.td}>{review.rating || 0}</td>
                  <td style={strapiStyles.td}>{review.reports?.length || 0}</td>
                  <td style={strapiStyles.td}>{review.created_at.slice(0, 10)}</td>
                  <td style={strapiStyles.td}>
                    <button
                      style={strapiStyles.primaryButton}
