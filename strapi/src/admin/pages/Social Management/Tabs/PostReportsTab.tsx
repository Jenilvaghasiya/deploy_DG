...
const handleDeleteReport = async (reportId: string, postId: string) => {
  try {
    await axios.delete(`${process.env.STRAPI_ADMIN_NODE_BACKEND_URL}/api/v1/strapi-admin/social-posts/report/delete/${postId}/${reportId}`, {
      headers: {
        "x-node-auth-token": process.env.STRAPI_ADMIN_NODE_AUTH_TOKEN,
      },
    });
    ...
    <div style={strapiStyles.container}>
      ...
      <div style={strapiStyles.contentBox}>
        ...
        <div style={strapiStyles.header}>
          <h2 style={strapiStyles.title}>Post Reports</h2>
        </div>
          ...
          <th style={strapiStyles.th}>Actions</th>
              ...
              {posts.map((post) => (
                <tr key={post._id} style={strapiStyles.tr}>
                  <td style={strapiStyles.td}>{post?.user_id?.nick_name || "Unknown"}</td>
                  ...
                  <td style={strapiStyles.td}>{post.reports?.length || 0}</td>
                  <td style={strapiStyles.td}>{new Date(post.created_at).toLocaleDateString()}</td>
                  <td style={strapiStyles.td}>
                    <button
