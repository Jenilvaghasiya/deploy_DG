...
import PatternCutoutListing from "./PatternCutoutListing";
import PatternCutouts from "./PatternCutouts";
import { useLocation } from "react-router-dom";

export default function PatternCutoutPage() {
  const location = useLocation();
  const defaultTabFromState = location.state?.defaultTab;
  const [activeTab, setActiveTab] = useState(defaultTabFromState || "listing");
console.log(defaultTabFromState);
          ...
          <div className="container">
            <TabsList className="grid w-full max-w-full grid-cols-2 h-11 sm:h-12 p-1 bg-black/30 backdrop-blur-md border border-white/10 rounded-xl shadow-lg">
              ...
              >
                ...
                <span className="hidden sm:inline">Cutout Listing</span>
              </TabsTrigger>

              {/* Generate Cutouts Tab */}
              <TabsTrigger
                value="generate"
                className="flex items-center justify-center gap-2 text-sm font-medium transition-all duration-200 cursor-pointer
                  data-[state=active]:bg-gradient-to-r data-[state=active]:from-purple-600/30 data-[state=active]:to-blue-600/30
              ...
              >
                ...
                <span className="hidden sm:inline">Generate Cutouts</span>
              </TabsTrigger>
            </TabsList>
          </div>
        </div>

        {/* Cutout Listing Content */}
        <TabsContent value="listing" className="flex-1 mt-0 overflow-auto custom-scroll">
          <div className="text-white p-4 lg:p-6 h-full">
            <div className="container space-y-4">
              ...
              <PatternCutoutListing />
            </div>
          </div>
        </TabsContent>

        {/* Generate Cutouts Content */}
